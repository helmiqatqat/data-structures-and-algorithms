...
const allUpperCase = (arr) => {
  arr.forEach((e,i,a) => a[i] = e.toUpperCase());
  return arr;
};
...
const speaker = (words, callback) => {
  words.forEach((e,i,a) => a[i] = callback(e));
  return words;
};
...
const createList = (availableItems) => {
  const list = [];
  availableItems.forEach((item) => item.available ? list.push(item.name) : 0);
  return list;
};
...
const fizzbuzz = (arr) => {
  arr.forEach((e,i,a) => e % 3 === 0 ? a[i] = 'Fizz' : e % 5 === 0 ? a[i] = 'Buzz' : e % 3 === 0 && e % 5 === 0 ? a[i] = 'Fizz Buzz' : a[i] = e);
  return arr;
};
