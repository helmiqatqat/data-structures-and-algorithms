...
const screenForNames = (arr) => {
  return arr.filter(e => /^(Mr\.|Ms\.|Mrs\.|Dr\.)\s{1}[a-zA-Z]+\b/.test(e));
};
...
const toTitleCase = (arr) => {
  return arr.map(e => e[0].toUpperCase().concat(e.slice(1)));
};
...
let biggerThanLuke = (arr) => {
  return arr.filter(e => +e.mass > +arr[0].mass).map(e => e.name).join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  const helpCheck = (a, b, c) => {
    return a === b && a === c && a !== '';
  };
  for(let i = 0; i < board.length; i++) {
    if (helpCheck(board[i][0], board[i][1], board[i][2]) || helpCheck(board[0][i], board[1][i], board[2][i])) {
      return true;
    }
  }
  if (helpCheck(board[0][0], board[1][1], board[2][2]) || helpCheck(board[0][2], board[1][1], board[2][0])) {
    return true;
  }
  return false;
};
