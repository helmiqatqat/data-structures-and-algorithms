...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => {
    return arr.filter(e => typeof e === 'number' && e % 5 === 0).map(e => Math.pow(2, e));
  });
};
...
let findMaleAndFemale = (data) => {
  const array = [];
  data.forEach(e => ['male','female'].includes(e.gender) ? array.push(e.name) : e);
...
let findShortest = (data) => {
  return data.filter(e => e.height === `${Math.min(...data.map(e => (e.height)))}`)[0].name;
};
