...
const hasChildrenValues = (arr, character) => {
  const charInfo = arr.filter(e => e.name === character)[0];
  return Object.values(charInfo).some(e => Array.isArray(e));
};
...
const hasChildrenEntries = (arr, character) => {
  const charInfo = arr.filter(e => e.name === character)[0];
  return charInfo ? Object.entries(charInfo)[2][0] === 'children' : false;
};
...
const totalCharacters = (arr) => {
  return arr.reduce((acc, char) => {
    return acc + 1 + (char.children ? char.children.length : 0) + (char.spouse ? 1 : 0);
  }, 0);
};
/* ------------------------------------------------------------------------------------------------
...
const houseSize = (arr) => {
  ...
  for(let char of arr) {
    sizes.push({
      house: char.house,
      members: 1 + (char.children ? char.children.length : 0) + (char.spouse ? 1 : 0)
    });
  }
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  for(let char of arr) {
    survivors.push({
      house: char.house,
      members: 1 + (char.children ? char.children.length : 0) + (char.spouse && !deceasedSpouses.includes(char.spouse) ? 1 : 0)
    });
  }
  return survivors;
};
