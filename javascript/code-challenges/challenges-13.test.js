...
const standardizePhoneNumbers = (arr) => {
  return arr.map(e => e.replace(/[^0-9]/g, ''));
};
...
const unenrollBrook = (arr) => {
  return arr.map(a => a.filter(e => !e.includes('Brook')));
};
...
const sortByDay = (arr) => {
  let sortedArray = Array(7).fill([]);
  for(let str of arr) {
    for(let day of daysOfWeek) {
      if(str.includes(day)) sortedArray[daysOfWeek.indexOf(day)] = str;
    }
  }
  return sortedArray;
};
